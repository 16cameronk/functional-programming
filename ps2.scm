...
;; Public keys
(define Rupert     (make-key 32455 108371))
(define Bruce      (make-key 30529 216221))
(define Hunter     (make-key 28859 135659))
(define Tyler      (make-key 28559 171967))
(define Dina       (make-key 27089 89701))
...
;; We understand that rsa-unconvert-list is the "reverse" of 
;; rsa-convert-list. This would mean an undoing of each step
;; that the latter does. 
;; 1. In rsa-convert-list, rsa-transform occurs after we add mod n
;; to the current block and the previous encrpytion block.
;; Thus, for rsa-unconvert-list, we should perform rsa-transform
;; BEFORE we subtract mod n of the previous encrpytion block.
;; 2. In rsa-convert-list, we add mod n of the current block to
;; the previous encryption block. So, for rsa-unconvert-list, we 
;; would subtract the mod n of the previous encryption block.
;; 3. After 2, we would then perform the inverse of the modulo 
;; function to obtain the plaintext message. This I believe is the 
;; smallest integer n that fulfills the inverse of a modulo function.
...
(define (encrypt-and-sign 
  msg sender-private-key recipient-public-key)
  (let ((ciphertext (rsa-encrypt msg recipient-public-key)))
    (make-signed-message ciphertext
      (car (rsa-convert-list (list (compress ciphertext))
        sender-private-key)))))

(define (decrypt-and-verify 
  signed-message sender-public-key recipient-private-key)
  (let* 
    ((plaintext (rsa-unconvert-list 
      (signed-message-body signed-message) recipient-private-key))
    (true-sig (compress (signed-message-body signed-message)))
    (given-sig (rsa-unconvert-list
      (list (signed-message-signature signed-message)) sender-public-key)))
    (if (= true-sig (car given-sig))
      (make-intlist-to-string-func plaintext)
      #f)))
...
;; (141024 48674 204177 339709 317361 307215 225252 330769 
;; 134147 215314 540102 3551 396788 42331 536293 214006 171543)
...
(define rupert-private-key (crack-rsa (key-pair-public Rupert)))
(define bruce-private-key (crack-rsa (key-pair-public Bruce)))
(define hunter-private-key (crack-rsa (key-pair-public Hunter)))
(define tyler-private-key (crack-rsa (key-pair-public Tyler)))
(define dina-private-key (crack-rsa (key-pair-public Dina)))

(rsa-decrypt secret-message rupert-private-key)
(rsa-decrypt secret-message bruce-private-key)
(rsa-decrypt secret-message hunter-private-key)
(rsa-decrypt secret-message tyler-private-key)
(rsa-decrypt secret-message dina-private-key)
  ...
  (lambda ((n <integer>) (base <integer>) (max <integer))
    (letrec ((iter
          (lambda ((d <integer>) (k <integer>))
            ...
            (else
             (iter 
              (gcd (- (modulo (exptmod base (factorial (- k 1)) n) n)  1) n) 
              (+ k 1)))))))
      (iter 1 1))))
